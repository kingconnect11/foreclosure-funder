      ...
      <table className="w-full text-left border-collapse whitespace-nowrap">
        <thead>
          <tr className="border-b-2 border-border">
            ...
            <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-[0.1em] text-text-secondary">Subscription</th>
              ...
              <React.Fragment key={investor.id}>
                ...
                >
                  ...
                  <td className="px-6 py-5">
                    <span className="inline-block px-2 py-1 rounded-lg text-[10px] tracking-[0.1em] font-bold uppercase border bg-background text-text-secondary border-border">
                      {investor.subscription_tier}
                    </span>
                  </td>
                  <td className="px-6 py-5 financial-value text-text-primary">{totalSaved}</td>
                  <td className="px-6 py-5 financial-value text-accent">{activeDeals}</td>
                  <td className="px-6 py-5 text-text-muted financial-value">
                    {investor.last_active ? new Date(investor.last_active).toLocaleDateString() : '--'}
                  </td>
                  ...
                  <tr className="bg-background border-b-2 border-border">
                    <td colSpan={7} className="px-6 py-8">
                      <div className="flex flex-col gap-4">
                        <span className="kicker">Pipeline Summary</span>
                          ...
                          <div className="flex flex-wrap gap-4">
                            {Object.entries(investor.summary).map(([stage, count]) => (
                              <div key={stage} className="flex items-center justify-between min-w-[160px] ledger-divider pb-1">
                                ...
                                <span className="financial-value text-sm text-text-primary">{count}</span>
                              </div>
                            ))}
                          </div>
                        ) : (
