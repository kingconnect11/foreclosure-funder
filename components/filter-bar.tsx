    ...
    <div className="zen-card p-4 lg:p-5">
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        ...
        <button
          ...
          className={clsx(
            'lg:hidden flex items-center justify-center gap-2 px-4 py-2.5 min-h-[44px] rounded-lg text-sm font-medium transition-all',
      ...
      {isExpanded && (
        <div className="lg:hidden mt-4 pt-4 border-t border-border grid grid-cols-1 md:grid-cols-3 gap-3 animate-fade-in">
          <select
            value={searchParams.get('stage') || ''}
            onChange={(e) => handleFilterChange('stage', e.target.value)}
            className="input-zen"
          ...
          {hasFilters && (
            <button
              onClick={clearFilters}
              className="flex items-center justify-center gap-2 btn-secondary md:col-span-3"
            >
              <X className="w-4 h-4" />
              Clear all filters
            </button>
